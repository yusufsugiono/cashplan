/**
 * ExpenseChart — Horizontal bar chart untuk pengeluaran per kategori.
 *
 * Props:
...
export default function ExpenseChart({ categories, onCategoryClick }) {
  ...
  return (
    <div className="px-4 py-4">
      <p className="text-sm text-[var(--color-muted)] mb-1">Total Pengeluaran</p>
      <p className="text-lg font-bold text-[var(--color-text)] mb-4">{formatRupiah(total)}</p>
      <div className="space-y-3">
        {categories.map((cat, idx) => (
          <div
            key={cat.category}
            onClick={() => onCategoryClick?.({ category: cat.category, type: 'EXPENSE' })}
